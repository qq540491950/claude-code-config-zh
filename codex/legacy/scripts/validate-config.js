...
const requiredFiles = [
  'README.md',
  'AGENTS.md',
  'CLAUDE-CODEX-MAPPING.md',
  'docs/配置定制指南.md',
  'hooks/hooks.json',
  'mcp-configs/mcp-servers.json',
  'scripts/validate-config.js',
  'scripts/copy-config.js',
  'scripts/sync-from-claude.js',
...
function validateAgentsEntry() {
  ...
  assert(entry.includes('配置代号（Config ID）'), 'AGENTS.md 缺少配置代号声明')
  assert(entry.includes('`UCC`'), 'AGENTS.md 缺少 UCC 标识')
  assert(entry.includes('/ucc-'), 'AGENTS.md 缺少 /ucc-* 命令约定')
  assert(entry.includes('配置标识：UCC'), 'AGENTS.md 缺少输出标识约定')
}
