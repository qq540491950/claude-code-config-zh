...
This skill is migrated from legacy command `ucc-sessions` and is now executed via Codex Skills.

## Trigger

- Explicit call: `$ucc-sessions`
...
管理 Codex 会话历史 - 列出、加载、别名和编辑存储在 `~/.codex/sessions/` 中的会话。
...
- 会话存储为 `~/.codex/sessions/` 中的 markdown 文件
- 别名存储在 `~/.codex/session-aliases.json` 中
